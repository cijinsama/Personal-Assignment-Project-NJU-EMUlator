...
#define immJ() do { *imm = SEXT(BITS(i, 32, 12), 20); *imm = ( (*imm & BITMASK_SELF(32, 20)) | ((*imm >> 9) & BITMASK_SELF(9, 1)) | (*imm & BITMASK_SELF(8, 1)) << 11 | (*imm & BITMASK_SELF(9,9)) << 2) << 1; } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();//INSTPAT左边为高位，右边为低位
	...
	INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , U, R(dest) = s->dnpc, s->dnpc += imm, printf("%08x\n",imm););								//注意，要求跳转数为2的倍数，并只记录21位的前20位
