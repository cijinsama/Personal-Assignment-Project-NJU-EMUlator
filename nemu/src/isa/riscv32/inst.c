...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11",	ecall	 , N, s->dnpc = isa_raise_intr(EXCP_Environment, s->pc));
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = csr.mepc, csr.mstatus.decode.MIE = csr.mstatus.decode.MPIE, csr.mstatus.decode.MPIE = 1; IFDEF(CONFIG_ETRACE, Log("[etrace] : mret to pc : %x", csr.mepc); log_write("[etrace] : mret to pc : %x", csr.mepc);));
	...
	INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(dest) = ((SEXT(src1, 32)) * (uint64_t)src2) >> 32);//这个可能有问题
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu	 , R, R(dest) = ((uint64_t)src1 * (uint64_t)src2) >> 32);//这个可能有问题
