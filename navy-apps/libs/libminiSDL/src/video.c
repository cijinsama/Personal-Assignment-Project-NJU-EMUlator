...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect) {
  ...
  assert(dst->format->BitsPerPixel == src->format->BitsPerPixel);
	...
	if(size_per_pixel == 1){
		...
		for (int i = 0; i < h; ++i){
				memcpy(dst_p + (dst_y + i) * dst->w, src_p + (src_y + i) * src->w, size_per_pixel * w);
		}
	}
		...
		for (int i = 0; i < h; ++i){
				memcpy(dst_p + (dst_y + i) * dst->w + dst_x, src_p + (src_y + i) * src->w + src_x, size_per_pixel * w);
		}
	}
	return;
}
...
uint32_t get_color(SDL_Palette *palette, int x, int y, SDL_Surface *s){
	 SDL_Color color = palette->colors[s->pixels[y * s->w + x]];
